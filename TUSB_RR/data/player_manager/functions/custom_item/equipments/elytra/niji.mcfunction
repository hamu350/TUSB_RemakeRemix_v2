#> player_manager:custom_item/equipments/elytra/niji
#
# 虹の翼
#
# @within function player_manager:custom_item/equipments/

effect give @s minecraft:regeneration 61 1 false

effect give @s minecraft:fire_resistance 61 0 false

effect give @s minecraft:haste 61 1 false

effect give @s minecraft:jump_boost 61 4 false

effect give @s minecraft:water_breathing 61 0 false

effect give @s minecraft:night_vision 61 0 false

effect give @s minecraft:resistance 61 2 false

particle dust 1 0 0 1 ~ ~1 ~ 1.4 1.4 1.4 10 2 force @a

particle dust 1 0.5 0 1 ~ ~1 ~ 1.4 1.4 1.4 10 2 force @a

particle dust 1 1 0 1 ~ ~1 ~ 1.4 1.4 1.4 10 2 force @a

particle dust 0 1 0 1 ~ ~1 ~ 1.4 1.4 1.4 10 2 force @a

particle dust 0 1 1 1 ~ ~1 ~ 1.4 1.4 1.4 10 2 force @a

particle dust 0 0 1 1 ~ ~1 ~ 1.4 1.4 1.4 10 2 force @a

particle dust 5 0 1 1 ~ ~1 ~ 1.4 1.4 1.4 10 2 force @a
