#> skill:job_skill/archer/chain_arrow/hit/
# チェインアローが直接当たった時
...
### チェインアローのレベルに応じて呼び出し
execute if score Level TUSB matches 0 as @e[distance=..5,type=#lib:mob,tag=Enemy,sort=random] at @s run function skill:job_skill/archer/chain_arrow/hit/damage
execute if score Level TUSB matches 1 as @e[distance=..15,type=#lib:mob,tag=Enemy,sort=random] at @s run function skill:job_skill/archer/chain_arrow/hit/damage
execute if score Level TUSB matches 2..9 as @e[distance=..25,type=#lib:mob,tag=Enemy,sort=random] at @s run function skill:job_skill/archer/chain_arrow/hit/damage
