#> skill:job_skill/ninja/choyaku/jump
#
### Copyright © 2022 赤石愛
...
execute if score @s Choyaku matches 2..7 as @e[distance=..3,type=#lib:mob,tag=Enemy] run function score_damage:api/attack
execute if score @s Choyaku matches 8..12 as @e[distance=..6,type=#lib:mob,tag=Enemy] run function score_damage:api/attack
execute if score @s Choyaku matches 13.. as @e[distance=..9,type=#lib:mob,tag=Enemy] run function score_damage:api/attack

execute if entity @e[type=#lib:living,distance=..9] run scoreboard players add @s HealCount 4