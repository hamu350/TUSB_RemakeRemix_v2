#> skill:job_skill/ninja/seiyou/apply
### 跳躍発動時の処理
...
# 軽減不可の0.5-1.0-2.0dmg
  ...
  # 値に応じてダメージを設定
    execute if score @s seiyou matches 1..12 run data modify storage score_damage: Argument.Damage set value 40
    execute if score @s seiyou matches 13..22 run data modify storage score_damage: Argument.Damage set value 80
    execute if score @s seiyou matches 23.. run data modify storage score_damage: Argument.Damage set value 120
