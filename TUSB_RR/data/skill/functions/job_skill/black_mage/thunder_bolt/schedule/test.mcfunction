#> skill:job_skill/black_mage/thunder_bolt/schedule/test
# サンダーボルトの範囲をテスト
### Copyright © 2022 赤石愛
...
### 水の有無によって範囲が変わる
execute if score @s TUSB = _ TUSB if entity @s[nbt={Fire:0s}] run function skill:job_skill/black_mage/thunder_bolt/schedule/invoke/all
execute if score @s TUSB = _ TUSB unless entity @s[nbt={Fire:0s}] run function skill:job_skill/black_mage/thunder_bolt/schedule/invoke/single
