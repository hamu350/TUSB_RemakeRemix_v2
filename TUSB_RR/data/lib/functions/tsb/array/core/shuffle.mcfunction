#> lib:tsb/array/core/shuffle
# @within function lib:tsb/array/**

execute store result score $Index Lib run function api:random/
scoreboard players operation $Index Lib %= $ListSize Lib
execute if score $Index Lib matches 1.. run function lib:tsb/array/core/move
...
execute if score $ListSize Lib matches 1.. run function lib:tsb/array/core/shuffle