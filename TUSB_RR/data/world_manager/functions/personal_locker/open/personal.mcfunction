#> world_manager:personal_locker/open/personal
# 
### Copyright © 2022 赤石愛
...
execute unless data storage oh_my_dat: _[-4][-4][-4][-4][-4][-4][-4][-4].TUSB.PersonalEnderChestSlot run function world_manager:personal_locker/open/init

### 通常のスロットは０
scoreboard players set _ TUSB 0
### もしも制圧済みのポータルの上だったら、個人ロッカーを有効化
execute if block ~ ~-1 ~ minecraft:end_portal_frame[eye=true] run function world_manager:personal_locker/open/activate
### サモンエンチェスや印板で呼び出されたものだったとしても個人ロッカーを有効化
execute align xyz if entity @e[dx=0,tag=PersonalLocker] run function world_manager:personal_locker/open/activate
