#> asset_manager:spawner/update/single
# 一種類のスポナーの置き換え

tellraw @a[tag=DebugInfo] {"translate":"function:asset_manager:spawner/update/single\nId:%s\nPos:%s","with":[{"score":{"name": "@s","objective": "SpawnerId"}},{"nbt": "Pos","entity": "@s"}]}
...
# 取得したデータを適応
execute if data storage asset: mob if predicate lib:spawner/is_vechicle_spawner on passengers run data modify entity @s {} merge from storage asset:context new_spawner
execute if data storage asset: mob if entity @s[tag=BlockSpawner] if block ~ ~ ~ spawner unless predicate lib:spawner/is_vechicle_spawner run data modify block ~ ~ ~ {} merge from storage asset:context new_spawner