...
## トカルトの処理
execute unless data storage tusb_remake: settings{toculting:1b} as @a[predicate=world_manager:area/tocult_colde,gamemode=!spectator] at @s run function world_manager:dimension/tocult_process/
execute unless data storage tusb_remake: settings{toculting:1b} as @a[predicate=!world_manager:area/tocult_colde,gamemode=!spectator] at @s if entity @e[tag=TocultArea,distance=..32] run function world_manager:dimension/tocult_process/
execute if data storage tusb_remake: settings{toculting:1b} as @a[predicate=!world_manager:area/nether_trial,gamemode=!spectator] at @s run function world_manager:dimension/tocult_process/

## ゾンビピッグマンのおこる対象を設定する
execute as @a[gamemode=!spectator] at @s run function world_manager:gimmic/nether/angry
...
## ホッパー島のワープ
execute positioned 177 39 -75 if entity @a[distance=..60,gamemode=!spectator] run function world_manager:gimmic/hopper/
...
# 煉獄マグマ
execute as @a[predicate=world_manager:area/nether_dungeon] unless score @s PurgatoryInvalid matches 1.. at @s if block ~ ~0.5 ~ minecraft:lava run function world_manager:dimension/rengoku_process/in_lava
