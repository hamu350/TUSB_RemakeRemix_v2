...
### 経験値取得処理
execute as @a[nbt={Inventory:[{id:"minecraft:nether_star"}]}] at @s run function player_manager:pick_exp/
...
### テーブルマウンテンの不思議な力
execute positioned -6 65 -2148 as @a[dx=64,dy=135,dz=48,gamemode=adventure] run function world_manager:gimmic/table_mountain/fall_force
execute as @a[predicate=world_manager:area/table_mountain,gamemode=!spectator] at @s if block ~ ~-0.5 ~ minecraft:redstone_block unless block ~ ~-1.5 ~ minecraft:command_block run function world_manager:gimmic/table_mountain/red_zone

###ネザー火山噴火
execute as @a[scores={MineNetherrack=1..}] at @s run function player_manager:volcano/

# ### サボテン島の罠 もとい サトウキビ島
execute positioned 39 7 67 if entity @a[distance=..10,limit=1] run function world_manager:gimmic/sealed_alter/
...
# ボスチーム
execute at @e[team=Boss] as @e[type=potion,distance=..3] run function core:clock/potion_barrier
...
### 個人ロッカー(印板)
execute as @e[tag=PersonalLockerSign] at @s run function core:clock/personal_locker_sign
