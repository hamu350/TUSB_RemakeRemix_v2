#> remake_remix:enemy/skill/game_master/2
#
# 
#
...
# 裏(1s後)
    execute as @e[tag=GM,limit=1] run execute if score @s Random matches 0 run schedule function remake_remix:enemy/skill/game_master/2/0 1s
# 表
    execute as @e[tag=GM,limit=1] run execute if score @s Random matches 1 run schedule function remake_remix:enemy/skill/game_master/2/1 1s
