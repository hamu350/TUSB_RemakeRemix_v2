#> remake_remix:spawner/update/
# スポナーのentityを置き換え

tellraw @a[tag=DebugInfo] {"translate":"function:remake_remix:spawner/update/\nId:%s\nPos:%s","with":[{"score":{"name": "@s","objective": "SpawnerId"}},{"nbt": "Pos","entity": "@s"}]}

# スポナーのデータを取得
function remake_remix:spawner/update/get_info/
...
execute if score @s SpawnerId matches -1 if data storage tusb_remake: {_:true} run function remake_remix:spawner/update/multi/
execute if score @s SpawnerId matches 1.. if data storage tusb_remake: {_:true} run function remake_remix:spawner/update/single/
